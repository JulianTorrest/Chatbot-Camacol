...
# -*- coding: utf-8 -*-
"""
NUEVA ESTRATEGIA - Generador de Documentos RAG desde Excel de Financiación de Vivienda (FIVI) del DANE

Este script lee el archivo Excel de "Financiación de Vivienda (FIVI)",
...
# 1. Ruta a la carpeta que contiene el archivo Excel de Financiación de Vivienda
EXCEL_FOLDER = Path(r"C:\Users\jytorres\OneDrive - CAMACOL\Documentos\Coordinación de Información Estrategica\Chatbot-Camacol-main\RAG\2025\DANE\Financiación de Vivienda (FIVI)")

# 2. Lista de los archivos Excel a procesar
EXCEL_FILES = [
    "anex-FIVI-SerieTipoBaseMun-IIItrim2025.xlsx"
]

# 3. Carpeta de destino para los documentos RAG generados
RAG_OUTPUT_FOLDER = Path(r"C:\Users\jytorres\OneDrive - CAMACOL\Documentos\Coordinación de Información Estrategica\Chatbot-Camacol-main\RAG\fivi_autogenerado")
...
def generar_qa_para_fila(row_data: pd.Series, sheet_name: str, file_name: str) -> str:
    """Usa un LLM para generar preguntas y respuestas a partir de una fila de datos de FIVI."""
    
    # Crear un contexto claro y conciso a partir de la fila
    contexto = f"Contexto de Financiación de Vivienda (FIVI) del DANE - {file_name} (Hoja: {sheet_name}):\n"
    for col, value in row_data.items():
        contexto += f"- {col}: {value}\n"

    prompt = f"""
    Eres un analista financiero experto en el mercado de financiación de vivienda en Colombia.
    Basado en el siguiente contexto de datos del DANE sobre Financiación de Vivienda (FIVI), genera 7 preguntas y respuestas variadas y naturales.
    Las preguntas deben ser como las haría un usuario final (ej: "¿Cuántos créditos se desembolsaron?", "dame el valor total de los préstamos...").
    ...
    P: ¿Cuál fue el número de créditos desembolsados para vivienda VIS en el tercer trimestre de 2025?
    R: En el III Trimestre de 2025, se desembolsaron 50,000 créditos para vivienda VIS.

    P: ¿Cuál fue el valor total de los créditos para vivienda No VIS?
    R: El valor total de los créditos para vivienda No VIS fue de 15.5 billones de pesos.

    P: ¿Cómo varió el número de créditos anualmente?
    R: El número de créditos tuvo una variación anual del 3.5%.
    ---
    ...
    titulo = f"# Financiación de Vivienda (FIVI): {sheet_name} - {row_data.get('Tipo de vivienda', 'N/A')}\n\n"
        ...
        for sheet_name in xls.sheet_names:
            ...
            for index, row in df.iterrows():
                ...
                if qa_block:
                    ...
                    tipo_vivienda = str(row.get('Tipo de vivienda', f'fila_{index}')).replace(' ', '_').replace('/', '_')
                    filename = f"{tipo_vivienda}.txt"
                    filepath = output_folder / filename
                    
                    with open(filepath, 'w', encoding='utf-8') as f:
                        f.write(qa_block)
                    
                    print(f"    ✅ Fila {index+1} ('{tipo_vivienda}') guardada en: {filepath.name}")
...
def main():
    """Función principal para generar todos los documentos RAG de Financiación de Vivienda."""
    
    print("🚀 Iniciando NUEVA ESTRATEGIA: Generación de RAG desde Excel de Financiación de Vivienda (DANE)...")
