...
# -*- coding: utf-8 -*-
"""
NUEVA ESTRATEGIA - Generador de Documentos RAG desde Excel de Proyecciones Macroeconómicas

Este script lee los archivos Excel de "Proyecciones Macroeconómicas" de Bancolombia,
...
# 1. Ruta a la carpeta que contiene los archivos Excel de Proyecciones
EXCEL_FOLDER = Path(r"C:\Users\jytorres\OneDrive - CAMACOL\Documentos\Coordinación de Información Estrategica\Chatbot-Camacol-main\RAG\2025\Bancolombia")

# 2. Lista de los archivos Excel a procesar
EXCEL_FILES = [
    "Proyecciones+Macroeconómicas+Colombia+-+Guía+2026.xlsx",
    "Proyecciones+Macroeconómicas+Colombia+-+Junio+2025.xlsx"
]

# 3. Carpeta de destino para los documentos RAG generados
RAG_OUTPUT_FOLDER = Path(r"C:\Users\jytorres\OneDrive - CAMACOL\Documentos\Coordinación de Información Estrategica\Chatbot-Camacol-main\RAG\macro_autogenerado")
...
def generar_qa_para_fila(row_data: pd.Series, sheet_name: str, file_name: str) -> str:
    ...
    contexto = f"Contexto de Proyecciones Macroeconómicas - {file_name} (Hoja: {sheet_name}):\n"
    for col, value in row_data.items():
        contexto += f"- {col}: {value}\n"

    prompt = f"""
    Eres un analista económico experto.
    Basado en el siguiente contexto de datos de proyecciones macroeconómicas para Colombia, genera 7 preguntas y respuestas variadas y naturales.
    Las preguntas deben ser como las haría un usuario final (ej: "¿Cuál es la proyección del PIB?", "dame la inflación esperada para...").
    ...
    P: ¿Cuál es la proyección de crecimiento del PIB para 2025?
    R: La proyección de crecimiento del PIB para 2025 es de 2.8%.

    P: ¿Qué se espera para la tasa de interés del Banco de la República?
    R: Se espera que la tasa de interés del Banco de la República se ubique en 7.5% para el cierre de 2025.

    P: ¿Cómo estará la inflación al final del año?
    R: La inflación al cierre de 2025 se proyecta en 4.5%.
    ---
    ...
    titulo = f"# Proyecciones Macroeconómicas: {sheet_name} - {row_data.get('Indicador', 'N/A')}\n\n"
...
def procesar_excel(file_path: Path):
    ...
    try:
        ...
        for sheet_name in xls.sheet_names:
            ...
            output_folder = RAG_OUTPUT_FOLDER / f"{file_path.stem}_{sheet_name}"
...
def main():
    """Función principal para generar todos los documentos RAG de proyecciones macroeconómicas."""
    
    print("🚀 Iniciando NUEVA ESTRATEGIA: Generación de RAG desde Excel de Proyecciones Macroeconómicas...")
